...
api: "GET https://api.worldlens.co/api/v1/simulations/{simulation_id}/"
...
```bash GeoJSON Export
curl -X GET "https://api.worldlens.co/api/v1/simulations/sim_abc123def456/?format=geojson" \
  -H "Authorization: Api-Key YOUR_API_KEY"
```

```bash 3D Model Export
curl -X GET "https://api.worldlens.co/api/v1/simulations/sim_abc123def456/?format=gltf&include_assets=true" \
  -H "Authorization: Api-Key YOUR_API_KEY"
```
</CodeGroup>
